...
<title>المقالات — هلا كارير</title>
...
html.light .ds-card,html.light .hc-card{background:#ffffff !important;border-color:rgba(0,212,184,.2) !important}
html.light [style*="rgba(5,14,26"]{background:#ffffff !important}
html.light [style*="rgba(4,11,22"]{background:#ffffff !important}
html.light [style*="rgba(5,16,30"]{background:#ffffff !important}
html.light [style*="rgba(3,13,23"]{background:#ffffff !important}
html.light [style*="background:rgba(0,0,0,.5)"]{background:rgba(0,0,0,.07) !important;color:#374151 !important}
html.light section{background:#ffffff !important}
html.light [style*="background:var(--bg)"]{background:#ffffff !important}
html.light [style*="background:var(--bg2)"]{background:#ffffff !important}
html.light [style*="background:var(--bg3)"]{background:#ffffff !important}
html.light [style*="color:var(--w)"] {color:#111827 !important}
html.light [style*="color:var(--w80)"]{color:#1f2937 !important}
html.light [style*="color:var(--w60)"]{color:#4b5563 !important}
html.light [style*="color:var(--w40)"]{color:#9ca3af !important}
...
html.light .hiw-icon{background:#ffffff;border-color:rgba(0,212,184,.2)}
...
/* ── Mobile filter popup ── */
#fp-overlay{position:fixed;inset:0;background:rgba(0,0,0,.75);z-index:800;
  opacity:0;visibility:hidden;transition:opacity .25s,visibility .25s;
  backdrop-filter:blur(6px);-webkit-backdrop-filter:blur(6px)}
#fp-overlay.open{opacity:1;visibility:visible}
#fp-panel{position:fixed;bottom:0;left:0;right:0;z-index:801;
  background:rgba(5,14,26,.98);border:1px solid var(--ta20);border-bottom:none;
...
html.light #fp-panel{background:#fff!important;border-color:rgba(0,212,184,.2)!important}
.fp-drag{width:40px;height:4px;border-radius:999px;
  background:rgba(255,255,255,.18);margin:0 auto 1.25rem;cursor:pointer}
html.light .fp-drag{background:rgba(0,0,0,.12)!important}

/* ── Filter btn — only on small ── */
#filter-btn{display:none}
@media(max-width:1023px){
  #filter-btn{display:inline-flex}
  aside.art-sidebar{display:none}
}
...
<!-- ── Search overlay (from diplomas) ── -->
<div id="search-overlay" onclick="if(event.target===this)closeSearch()">
  <div id="search-box">
    <div class="flex items-center gap-3 p-4">
      <svg class="w-5 h-5 shrink-0" style="color:var(--w40)" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="11" cy="11" r="8"/><path d="m21 21-4.35-4.35"/></svg>
      <input id="search-input" type="text" placeholder="ابحث في المقالات..." class="flex-1 bg-transparent border-none outline-none text-base" style="color:var(--w)">
      <button onclick="closeSearch()" class="text-[.7rem] font-bold px-3 py-1.5 rounded-lg" style="background:var(--ta08);color:var(--w60)">ESC</button>
    </div>
  </div>
</div>
...
<section class="relative pt-28 pb-16 overflow-hidden" style="background:var(--bg2)">
  ...
  <div class="relative z-10 max-w-[1280px] mx-auto px-5 text-center">
    ...
    <div class="sec-eyebrow mb-3 animate-fadeup">مركز المعرفة · هلا كارير</div>

    <h1 class="font-display font-extrabold tracking-tight mb-4 animate-fadeup" style="font-size:clamp(2.2rem,4.5vw,3.5rem);color:var(--w);animation-delay:.06s">
      <span>اقرأ ما يحرّك</span><br>
      <span class="grad">مسيرتك المهنية.</span>
    </h1>

    <p class="text-base max-w-xl mx-auto mb-8 leading-relaxed animate-fadeup" style="color:var(--w60);animation-delay:.12s">
      معرفة عملية مكتوبة من ممارسين. ليست مقالات أكاديمية — بل تجارب حقيقية من السوق.
    </p>

    <!-- Search bar + filter btn (mobile) -->
    <div class="flex flex-col sm:flex-row gap-3 max-w-2xl mx-auto mb-10 animate-fadeup" style="animation-delay:.18s">
      <div class="relative flex-1">
        ...
        <input class="hc-input ps-10 py-3" id="search-inp"
          placeholder="ابحث في المقالات..." oninput="doSearch(this.value)">
      </div>
      <button class="btn-p px-7 py-3 text-sm font-bold">بحث</button>
      <!-- Filter button — mobile only -->
      <button id="filter-btn" class="btn-g px-5 py-3 text-sm font-bold gap-2"
        onclick="openFP()">
        <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round"><path d="M22 3H2l8 9.46V19l4 2V12.46L22 3z"/></svg>
        الفلاتر
        <span id="fp-badge" style="display:none;width:18px;height:18px;border-radius:50%;
          background:var(--tl);color:#020e18;font-size:.6rem;font-weight:800;
          align-items:center;justify-content:center">0</span>
      </button>
    </div>

    <!-- Impact badges -->
    <div class="flex flex-wrap justify-center gap-3 animate-fadeup" style="animation-delay:.24s">
      <div class="flex items-center gap-2.5 px-5 py-3 rounded-2xl" style="background:rgba(5,16,30,.8);border:1px solid var(--ta20)">
        <span style="font-size:1.1rem">✍️</span>
        <span class="text-sm font-semibold" style="color:var(--w80)">كُتبت من الخبرة — لا من الكتب</span>
      </div>
      <div class="flex items-center gap-2.5 px-5 py-3 rounded-2xl" style="background:rgba(5,16,30,.8);border:1px solid var(--ta20)">
        <span style="font-size:1.1rem">⚡</span>
        <span class="text-sm font-semibold" style="color:var(--w80)">تطبّقها في نفس اليوم</span>
      </div>
      <div class="flex items-center gap-2.5 px-5 py-3 rounded-2xl" style="background:rgba(5,16,30,.8);border:1px solid var(--ta20)">
        <span class="font-display font-extrabold text-lg grad leading-none">٢٤ك+</span>
        <span class="text-sm font-semibold" style="color:var(--w80)">قارئ نشط شهرياً</span>
      </div>
    </div>
  </div>
</section>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-[260px,1fr] gap-8">
      ...
      <aside class="art-sidebar">
        <div class="hc-card p-6 sticky top-20">
          ...
          <div class="flex items-center justify-between mb-5">
            <div class="font-display font-bold text-sm" style="color:var(--w)">الفلاتر</div>
            <button class="text-[.65rem] font-bold" style="color:var(--t)" onclick="resetFilters()">مسح الكل</button>
          </div>

          <!-- التصنيف -->
          <div class="mb-6">
            <div class="text-[.65rem] font-bold tracking-[.14em] uppercase mb-3" style="color:var(--w40)">التصنيف</div>
            <div class="flex flex-col gap-2">
              <label class="flex items-center gap-3 cursor-pointer group" onclick="filterCat('all',this)">
                <div class="w-4 h-4 rounded border-2 flex items-center justify-center cat-indicator" id="ci-all" style="border-color:var(--t);background:var(--t)">
                  <svg class="w-2.5 h-2.5" viewBox="0 0 10 10" fill="none" stroke="#020e18" stroke-width="2.5"><path d="M1.5 5l2.5 2.5 4.5-4.5"/></svg>
                </div>
                <span class="text-sm group-hover:opacity-100 opacity-80" style="color:var(--w)">كل التصنيفات</span>
                <span class="ms-auto text-[.65rem] font-bold px-2 py-0.5 rounded-full" id="cnt-all" style="background:var(--ta12);color:var(--t)">٩</span>
              </label>
              <label class="flex items-center gap-3 cursor-pointer group" onclick="filterCat('career',this)">
                <div class="w-4 h-4 rounded border-2 flex items-center justify-center cat-indicator" id="ci-career" style="border-color:#2de0ca55;background:transparent"></div>
                <span class="text-sm group-hover:opacity-100 opacity-70" style="color:var(--w)">المسار المهني</span>
                <span class="ms-auto text-[.65rem] font-bold px-2 py-0.5 rounded-full" id="cnt-career" style="background:rgba(0,212,184,.08);color:#2de0ca"></span>
              </label>
              <label class="flex items-center gap-3 cursor-pointer group" onclick="filterCat('sales',this)">
                <div class="w-4 h-4 rounded border-2 flex items-center justify-center cat-indicator" id="ci-sales" style="border-color:#60a5fa55;background:transparent"></div>
                <span class="text-sm group-hover:opacity-100 opacity-70" style="color:var(--w)">مبيعات</span>
                <span class="ms-auto text-[.65rem] font-bold px-2 py-0.5 rounded-full" id="cnt-sales" style="background:rgba(96,165,250,.08);color:#60a5fa"></span>
              </label>
              <label class="flex items-center gap-3 cursor-pointer group" onclick="filterCat('hr',this)">
                <div class="w-4 h-4 rounded border-2 flex items-center justify-center cat-indicator" id="ci-hr" style="border-color:#c084fc55;background:transparent"></div>
                <span class="text-sm group-hover:opacity-100 opacity-70" style="color:var(--w)">موارد بشرية</span>
                <span class="ms-auto text-[.65rem] font-bold px-2 py-0.5 rounded-full" id="cnt-hr" style="background:rgba(192,132,252,.08);color:#c084fc"></span>
              </label>
              <label class="flex items-center gap-3 cursor-pointer group" onclick="filterCat('real',this)">
                <div class="w-4 h-4 rounded border-2 flex items-center justify-center cat-indicator" id="ci-real" style="border-color:#fb923c55;background:transparent"></div>
                <span class="text-sm group-hover:opacity-100 opacity-70" style="color:var(--w)">عقارات</span>
                <span class="ms-auto text-[.65rem] font-bold px-2 py-0.5 rounded-full" id="cnt-real" style="background:rgba(251,146,60,.08);color:#fb923c"></span>
              </label>
              <label class="flex items-center gap-3 cursor-pointer group" onclick="filterCat('lang',this)">
                <div class="w-4 h-4 rounded border-2 flex items-center justify-center cat-indicator" id="ci-lang" style="border-color:#34d39955;background:transparent"></div>
                <span class="text-sm group-hover:opacity-100 opacity-70" style="color:var(--w)">لغات</span>
                <span class="ms-auto text-[.65rem] font-bold px-2 py-0.5 rounded-full" id="cnt-lang" style="background:rgba(52,211,153,.08);color:#34d399"></span>
              </label>
              <label class="flex items-center gap-3 cursor-pointer group" onclick="filterCat('edu',this)">
                <div class="w-4 h-4 rounded border-2 flex items-center justify-center cat-indicator" id="ci-edu" style="border-color:#f472b655;background:transparent"></div>
                <span class="text-sm group-hover:opacity-100 opacity-70" style="color:var(--w)">تعليم</span>
                <span class="ms-auto text-[.65rem] font-bold px-2 py-0.5 rounded-full" id="cnt-edu" style="background:rgba(244,114,182,.08);color:#f472b6"></span>
              </label>
            </div>
          </div>
          ...
          <!-- وقت القراءة -->
          <div class="mb-6">
            <div class="text-[.65rem] font-bold tracking-[.14em] uppercase mb-3" style="color:var(--w40)">وقت القراءة</div>
            <div class="flex flex-col gap-2">
              <label class="flex items-center gap-3 cursor-pointer"><input type="radio" name="rt" value="all" checked class="accent-[#00d4b8]" onchange="filterTime('all')"><span class="text-sm" style="color:var(--w60)">أي وقت</span></label>
              <label class="flex items-center gap-3 cursor-pointer"><input type="radio" name="rt" value="short" class="accent-[#00d4b8]" onchange="filterTime('short')"><span class="text-sm" style="color:var(--w60)">سريع — أقل من ٨ د</span></label>
              <label class="flex items-center gap-3 cursor-pointer"><input type="radio" name="rt" value="long" class="accent-[#00d4b8]" onchange="filterTime('long')"><span class="text-sm" style="color:var(--w60)">مطوّل — أكثر من ١٠ د</span></label>
            </div>
          </div>

          <!-- نشرة بريدية -->
          <div class="rounded-xl p-4" style="background:var(--ta08);border:1px solid var(--ta12)">
            <div class="text-[.7rem] font-bold mb-1" style="color:var(--w)">لا تفوت مقالاً</div>
            <div class="text-[.65rem] mb-3" style="color:var(--w40)">اشترك للنشرة الأسبوعية</div>
            <input class="hc-input text-[.8rem] py-2 mb-2" type="email" placeholder="بريدك الإلكتروني">
            <button class="btn-p w-full py-2 text-[.78rem]">اشترك</button>
          </div>
        </div>
      </aside>
      ...
      <div>
        ...
        <div class="flex flex-col sm:flex-row gap-3 items-start sm:items-center justify-between mb-6">
          <div class="flex gap-2 flex-wrap chip-group" id="chip-bar">
            <span class="chip on" data-cat="all" onclick="chipCat(this,'all')">الكل (<span id="cnt-chip-all">٩</span>)</span>
            <span class="chip" data-cat="career" onclick="chipCat(this,'career')">المسار المهني</span>
            <span class="chip" data-cat="sales" onclick="chipCat(this,'sales')">مبيعات</span>
            <span class="chip" data-cat="hr" onclick="chipCat(this,'hr')">موارد بشرية</span>
            <span class="chip" data-cat="real" onclick="chipCat(this,'real')">عقارات</span>
            <span class="chip" data-cat="lang" onclick="chipCat(this,'lang')">لغات</span>
            <span class="chip" data-cat="edu" onclick="chipCat(this,'edu')">تعليم</span>
          </div>
          <select class="hc-input w-auto text-sm py-2" style="cursor:pointer;min-width:140px">
            <option>الأحدث أولاً</option>
            <option>الأكثر قراءة</option>
            <option>الأقصر قراءة</option>
          </select>
        </div>

        <!-- Count -->
        <div class="text-[.78rem] mb-5" style="color:var(--w40)">
          <span class="font-display font-bold" style="color:var(--tl);font-size:1rem" id="art-count">٩</span>
          <span> مقال</span>
        </div>

        <!-- Cards grid -->
        <div class="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-5" id="art-grid">
<article class="ds-card art-card flex flex-col cursor-pointer" data-cat="career"
  style="transition:border-color .2s,transform .2s"
  onmouseover="this.style.transform='translateY(-3px)'"
  onmouseout="this.style.transform=''">
  <div style="height:3px;background:linear-gradient(90deg,#2de0ca,#2de0ca44,transparent)"></div>
  <div class="p-5 flex flex-col gap-3 flex-1">
    <div class="flex items-center justify-between">
      <span style="font-size:.62rem;font-weight:700;padding:.2rem .65rem;border-radius:999px;
        background:#2de0ca18;border:1px solid #2de0ca35;color:#2de0ca">المسار المهني</span>
      <span class="text-[.62rem]" style="color:var(--w40)">⏱ ٨ د</span>
    </div>
    <h3 class="font-display font-bold leading-snug flex-1"
      style="font-size:.97rem;color:var(--w);
        display:-webkit-box;-webkit-line-clamp:2;-webkit-box-orient:vertical;overflow:hidden">كيف تبني سيرة ذاتية تفتح أبواب الشركات الكبرى</h3>
    <p style="font-size:.79rem;line-height:1.65;color:var(--w60);
      display:-webkit-box;-webkit-line-clamp:2;-webkit-box-orient:vertical;overflow:hidden">تعرّف على الأسرار التي يستخدمها محترفو الـ HR لتقييم السير الذاتية في ثوانٍ معدودة.</p>
    <div class="flex items-center justify-between pt-3" style="border-top:1px solid var(--ta08)">
      <div class="flex items-center gap-2">
        <div style="width:24px;height:24px;border-radius:50%;flex-shrink:0;
          background:#2de0ca18;border:1px solid #2de0ca30;
          display:flex;align-items:center;justify-content:center;
          font-size:.58rem;font-weight:700;color:#2de0ca">أم</div>
        <span class="text-[.7rem]" style="color:var(--w60)">أحمد مصطفى</span>
      </div>
      <div class="flex items-center gap-2">
        <span class="text-[.62rem]" style="color:var(--w40)">👁 ٢.٤ك</span>
        <a href="hc-article.html" class="btn-g text-[.72rem] font-bold px-2.5 py-1 rounded-lg"
...
<article class="ds-card art-card flex flex-col cursor-pointer" data-cat="career"
  ...
  <div class="p-5 flex flex-col gap-3 flex-1">
    <div class="flex items-center justify-between">
      <span style="font-size:.62rem;font-weight:700;padding:.2rem .65rem;border-radius:999px;
        background:#2de0ca18;border:1px solid #2de0ca35;color:#2de0ca">المسار المهني</span>
      <span class="text-[.62rem]" style="color:var(--w40)">⏱ ١٢ د</span>
    </div>
    <h3 class="font-display font-bold leading-snug flex-1"
      style="font-size:.97rem;color:var(--w);
        display:-webkit-box;-webkit-line-clamp:2;-webkit-box-orient:vertical;overflow:hidden">دليلك الكامل للتفاوض على الراتب في مصر والخليج</h3>
    <p style="font-size:.79rem;line-height:1.65;color:var(--w60);
      display:-webkit-box;-webkit-line-clamp:2;-webkit-box-orient:vertical;overflow:hidden">الراتب لا يُعطى بل يُفاوَض عليه. اكتشف الأرقام الحقيقية لكل مجال.</p>
    <div class="flex items-center justify-between pt-3" style="border-top:1px solid var(--ta08)">
      <div class="flex items-center gap-2">
        <div style="width:24px;height:24px;border-radius:50%;flex-shrink:0;
          background:#2de0ca18;border:1px solid #2de0ca30;
          display:flex;align-items:center;justify-content:center;
          font-size:.58rem;font-weight:700;color:#2de0ca">سخ</div>
        <span class="text-[.7rem]" style="color:var(--w60)">سارة خليل</span>
      </div>
      <div class="flex items-center gap-2">
        <span class="text-[.62rem]" style="color:var(--w40)">👁 ١.٨ك</span>
        <a href="hc-article.html" class="btn-g text-[.72rem] font-bold px-2.5 py-1 rounded-lg"
          onclick="event.stopPropagation()">اقرأ ←</a>
      </div>
    </div>
...
<article class="ds-card art-card flex flex-col cursor-pointer" data-cat="sales"
  style="transition:border-color .2s,transform .2s"
  onmouseover="this.style.transform='translateY(-3px)'"
  onmouseout="this.style.transform=''">
  <div style="height:3px;background:linear-gradient(90deg,#60a5fa,#60a5fa44,transparent)"></div>
  <div class="p-5 flex flex-col gap-3 flex-1">
    <div class="flex items-center justify-between">
      <span style="font-size:.62rem;font-weight:700;padding:.2rem .65rem;border-radius:999px;
        background:#60a5fa18;border:1px solid #60a5fa35;color:#60a5fa">مبيعات</span>
      <span class="text-[.62rem]" style="color:var(--w40)">⏱ ١٠ د</span>
    </div>
    <h3 class="font-display font-bold leading-snug flex-1"
      style="font-size:.97rem;color:var(--w);
        display:-webkit-box;-webkit-line-clamp:2;-webkit-box-orient:vertical;overflow:hidden">٧ مهارات مبيعات B2B يطلبها سوق العمل الآن</h3>
    <p style="font-size:.79rem;line-height:1.65;color:var(--w60);
      display:-webkit-box;-webkit-line-clamp:2;-webkit-box-orient:vertical;overflow:hidden">قائمة المهارات التي يبحث عنها مدراء المبيعات في أكبر الشركات.</p>
    <div class="flex items-center justify-between pt-3" style="border-top:1px solid var(--ta08)">
      <div class="flex items-center gap-2">
        <div style="width:24px;height:24px;border-radius:50%;flex-shrink:0;
          background:#60a5fa18;border:1px solid #60a5fa30;
          display:flex;align-items:center;justify-content:center;
          font-size:.58rem;font-weight:700;color:#60a5fa">عف</div>
        <span class="text-[.7rem]" style="color:var(--w60)">عمر فريد</span>
      </div>
      <div class="flex items-center gap-2">
        <span class="text-[.62rem]" style="color:var(--w40)">👁 ٣.١ك</span>
        <a href="hc-article.html" class="btn-g text-[.72rem] font-bold px-2.5 py-1 rounded-lg"
          onclick="event.stopPropagation()">اقرأ ←</a>
      </div>
    </div>
...
<article class="ds-card art-card flex flex-col cursor-pointer" data-cat="hr"
  style="transition:border-color .2s,transform .2s"
  onmouseover="this.style.transform='translateY(-3px)'"
  onmouseout="this.style.transform=''">
  <div style="height:3px;background:linear-gradient(90deg,#c084fc,#c084fc44,transparent)"></div>
  <div class="p-5 flex flex-col gap-3 flex-1">
    <div class="flex items-center justify-between">
      <span style="font-size:.62rem;font-weight:700;padding:.2rem .65rem;border-radius:999px;
        background:#c084fc18;border:1px solid #c084fc35;color:#c084fc">موارد بشرية</span>
      <span class="text-[.62rem]" style="color:var(--w40)">⏱ ٩ د</span>
    </div>
    <h3 class="font-display font-bold leading-snug flex-1"
      style="font-size:.97rem;color:var(--w);
        display:-webkit-box;-webkit-line-clamp:2;-webkit-box-orient:vertical;overflow:hidden">أخطاء الـ HR التي تكلف الشركات ملايين</h3>
    <p style="font-size:.79rem;line-height:1.65;color:var(--w60);
      display:-webkit-box;-webkit-line-clamp:2;-webkit-box-orient:vertical;overflow:hidden">دراسة لأكثر الأخطاء تكراراً في التوظيف والاحتفاظ بالموظفين.</p>
    <div class="flex items-center justify-between pt-3" style="border-top:1px solid var(--ta08)">
      <div class="flex items-center gap-2">
        <div style="width:24px;height:24px;border-radius:50%;flex-shrink:0;
          background:#c084fc18;border:1px solid #c084fc30;
          display:flex;align-items:center;justify-content:center;
          font-size:.58rem;font-weight:700;color:#c084fc">نح</div>
        <span class="text-[.7rem]" style="color:var(--w60)">نور حسن</span>
      </div>
      <div class="flex items-center gap-2">
        <span class="text-[.62rem]" style="color:var(--w40)">👁 ٩٨٠</span>
        <a href="hc-article.html" class="btn-g text-[.72rem] font-bold px-2.5 py-1 rounded-lg"
          onclick="event.stopPropagation()">اقرأ ←</a>
      </div>
    </div>
...
<article class="ds-card art-card flex flex-col cursor-pointer" data-cat="real"
  style="transition:border-color .2s,transform .2s"
  onmouseover="this.style.transform='translateY(-3px)'"
  onmouseout="this.style.transform=''">
  <div style="height:3px;background:linear-gradient(90deg,#fb923c,#fb923c44,transparent)"></div>
  <div class="p-5 flex flex-col gap-3 flex-1">
    <div class="flex items-center justify-between">
      <span style="font-size:.62rem;font-weight:700;padding:.2rem .65rem;border-radius:999px;
        background:#fb923c18;border:1px solid #fb923c35;color:#fb923c">عقارات</span>
      <span class="text-[.62rem]" style="color:var(--w40)">⏱ ١١ د</span>
    </div>
    <h3 class="font-display font-bold leading-snug flex-1"
      style="font-size:.97rem;color:var(--w);
        display:-webkit-box;-webkit-line-clamp:2;-webkit-box-orient:vertical;overflow:hidden">كيف تبدأ في سوق العقارات بدون خبرة</h3>
    <p style="font-size:.79rem;line-height:1.65;color:var(--w60);
      display:-webkit-box;-webkit-line-clamp:2;-webkit-box-orient:vertical;overflow:hidden">خارطة الطريق الحقيقية لدخول سوق العقارات من الصفر.</p>
    <div class="flex items-center justify-between pt-3" style="border-top:1px solid var(--ta08)">
      <div class="flex items-center gap-2">
        <div style="width:24px;height:24px;border-radius:50%;flex-shrink:0;
          background:#fb923c18;border:1px solid #fb923c30;
          display:flex;align-items:center;justify-content:center;
          font-size:.58rem;font-weight:700;color:#fb923c">كع</div>
        <span class="text-[.7rem]" style="color:var(--w60)">كريم عادل</span>
      </div>
      <div class="flex items-center gap-2">
        <span class="text-[.62rem]" style="color:var(--w40)">👁 ١.٢ك</span>
        <a href="hc-article.html" class="btn-g text-[.72rem] font-bold px-2.5 py-1 rounded-lg"
          onclick="event.stopPropagation()">اقرأ ←</a>
      </div>
    </div>
...
<article class="ds-card art-card flex flex-col cursor-pointer" data-cat="lang"
  style="transition:border-color .2s,transform .2s"
  onmouseover="this.style.transform='translateY(-3px)'"
  onmouseout="this.style.transform=''">
  <div style="height:3px;background:linear-gradient(90deg,#34d399,#34d39944,transparent)"></div>
  <div class="p-5 flex flex-col gap-3 flex-1">
    <div class="flex items-center justify-between">
      <span style="font-size:.62rem;font-weight:700;padding:.2rem .65rem;border-radius:999px;
        background:#34d39918;border:1px solid #34d39935;color:#34d399">لغات</span>
      <span class="text-[.62rem]" style="color:var(--w40)">⏱ ٧ د</span>
    </div>
    <h3 class="font-display font-bold leading-snug flex-1"
      style="font-size:.97rem;color:var(--w);
        display:-webkit-box;-webkit-line-clamp:2;-webkit-box-orient:vertical;overflow:hidden">تعلم الإنجليزية للعمل في ٩٠ يوماً</h3>
    <p style="font-size:.79rem;line-height:1.65;color:var(--w60);
      display:-webkit-box;-webkit-line-clamp:2;-webkit-box-orient:vertical;overflow:hidden">برنامج مكثف لاتقان الإنجليزية المهنية بمصادر مجانية ومدفوعة.</p>
    <div class="flex items-center justify-between pt-3" style="border-top:1px solid var(--ta08)">
      <div class="flex items-center gap-2">
        <div style="width:24px;height:24px;border-radius:50%;flex-shrink:0;
          background:#34d39918;border:1px solid #34d39930;
          display:flex;align-items:center;justify-content:center;
          font-size:.58rem;font-weight:700;color:#34d399">رم</div>
        <span class="text-[.7rem]" style="color:var(--w60)">رانيا مصطفى</span>
      </div>
      <div class="flex items-center gap-2">
        <span class="text-[.62rem]" style="color:var(--w40)">👁 ٥.٦ك</span>
        <a href="hc-article.html" class="btn-g text-[.72rem] font-bold px-2.5 py-1 rounded-lg"
          onclick="event.stopPropagation()">اقرأ ←</a>
      </div>
    </div>
...
<article class="ds-card art-card flex flex-col cursor-pointer" data-cat="edu"
  style="transition:border-color .2s,transform .2s"
  onmouseover="this.style.transform='translateY(-3px)'"
  onmouseout="this.style.transform=''">
  <div style="height:3px;background:linear-gradient(90deg,#f472b6,#f472b644,transparent)"></div>
  <div class="p-5 flex flex-col gap-3 flex-1">
    <div class="flex items-center justify-between">
      <span style="font-size:.62rem;font-weight:700;padding:.2rem .65rem;border-radius:999px;
        background:#f472b618;border:1px solid #f472b635;color:#f472b6">تعليم</span>
      <span class="text-[.62rem]" style="color:var(--w40)">⏱ ١٤ د</span>
    </div>
    <h3 class="font-display font-bold leading-snug flex-1"
      style="font-size:.97rem;color:var(--w);
        display:-webkit-box;-webkit-line-clamp:2;-webkit-box-orient:vertical;overflow:hidden">مستقبل التعليم: هل الشهادة الجامعية كافية؟</h3>
    <p style="font-size:.79rem;line-height:1.65;color:var(--w60);
      display:-webkit-box;-webkit-line-clamp:2;-webkit-box-orient:vertical;overflow:hidden">جدل حقيقي بين الشهادات الجامعية والدبلومات المهنية بأرقام.</p>
    <div class="flex items-center justify-between pt-3" style="border-top:1px solid var(--ta08)">
      <div class="flex items-center gap-2">
        <div style="width:24px;height:24px;border-radius:50%;flex-shrink:0;
          background:#f472b618;border:1px solid #f472b630;
          display:flex;align-items:center;justify-content:center;
          font-size:.58rem;font-weight:700;color:#f472b6">دس</div>
        <span class="text-[.7rem]" style="color:var(--w60)">دينا سمير</span>
      </div>
      <div class="flex items-center gap-2">
        <span class="text-[.62rem]" style="color:var(--w40)">👁 ٤.٣ك</span>
        <a href="hc-article.html" class="btn-g text-[.72rem] font-bold px-2.5 py-1 rounded-lg"
          onclick="event.stopPropagation()">اقرأ ←</a>
      </div>
    </div>
...
<article class="ds-card art-card flex flex-col cursor-pointer" data-cat="real"
  ...
  <div class="p-5 flex flex-col gap-3 flex-1">
    <div class="flex items-center justify-between">
      <span style="font-size:.62rem;font-weight:700;padding:.2rem .65rem;border-radius:999px;
        background:#fb923c18;border:1px solid #fb923c35;color:#fb923c">عقارات</span>
      <span class="text-[.62rem]" style="color:var(--w40)">⏱ ١٣ د</span>
    </div>
    <h3 class="font-display font-bold leading-snug flex-1"
      style="font-size:.97rem;color:var(--w);
        display:-webkit-box;-webkit-line-clamp:2;-webkit-box-orient:vertical;overflow:hidden">دليل إنشاء أول وكالة عقارية ناجحة</h3>
    <p style="font-size:.79rem;line-height:1.65;color:var(--w60);
      display:-webkit-box;-webkit-line-clamp:2;-webkit-box-orient:vertical;overflow:hidden">الخطوات القانونية والتشغيلية لبدء وكالة عقارية من الصفر.</p>
    <div class="flex items-center justify-between pt-3" style="border-top:1px solid var(--ta08)">
      <div class="flex items-center gap-2">
        <div style="width:24px;height:24px;border-radius:50%;flex-shrink:0;
          background:#fb923c18;border:1px solid #fb923c30;
          display:flex;align-items:center;justify-content:center;
          font-size:.58rem;font-weight:700;color:#fb923c">حع</div>
        <span class="text-[.7rem]" style="color:var(--w60)">حسن علي</span>
      </div>
      <div class="flex items-center gap-2">
        <span class="text-[.62rem]" style="color:var(--w40)">👁 ٧٦٠</span>
        <a href="hc-article.html" class="btn-g text-[.72rem] font-bold px-2.5 py-1 rounded-lg"
          onclick="event.stopPropagation()">اقرأ ←</a>
      </div>
    </div>
...
<article class="ds-card art-card flex flex-col cursor-pointer" data-cat="career"
  ...
  <div class="p-5 flex flex-col gap-3 flex-1">
    <div class="flex items-center justify-between">
      <span style="font-size:.62rem;font-weight:700;padding:.2rem .65rem;border-radius:999px;
        background:#2de0ca18;border:1px solid #2de0ca35;color:#2de0ca">المسار المهني</span>
      <span class="text-[.62rem]" style="color:var(--w40)">⏱ ٦ د</span>
    </div>
    <h3 class="font-display font-bold leading-snug flex-1"
      style="font-size:.97rem;color:var(--w);
        display:-webkit-box;-webkit-line-clamp:2;-webkit-box-orient:vertical;overflow:hidden">قوة الـ LinkedIn في سوق العمل العربي</h3>
    <p style="font-size:.79rem;line-height:1.65;color:var(--w60);
      display:-webkit-box;-webkit-line-clamp:2;-webkit-box-orient:vertical;overflow:hidden">كيف يستخدم المحترفون الناجحون LinkedIn لجذب الفرص والعروض.</p>
    <div class="flex items-center justify-between pt-3" style="border-top:1px solid var(--ta08)">
      <div class="flex items-center gap-2">
        <div style="width:24px;height:24px;border-radius:50%;flex-shrink:0;
          background:#2de0ca18;border:1px solid #2de0ca30;
          display:flex;align-items:center;justify-content:center;
          font-size:.58rem;font-weight:700;color:#2de0ca">ين</div>
        <span class="text-[.7rem]" style="color:var(--w60)">يارا نبيل</span>
      </div>
      <div class="flex items-center gap-2">
        <span class="text-[.62rem]" style="color:var(--w40)">👁 ٢.٩ك</span>
        <a href="hc-article.html" class="btn-g text-[.72rem] font-bold px-2.5 py-1 rounded-lg"
          onclick="event.stopPropagation()">اقرأ ←</a>
      </div>
    </div>
        ...
        <div id="empty" class="hidden text-center py-16">
          ...
          <div class="font-display font-bold text-lg mb-1" style="color:var(--w60)">لا توجد مقالات</div>
          <div class="text-sm" style="color:var(--w40)">جرب فلتراً مختلفاً</div>
        </div>
      </div>
    </div>
  </div>
...
<!-- ═══ MOBILE FILTER POPUP ═══ -->
<div id="fp-overlay" onclick="closeFP()"></div>
<div id="fp-panel">
  <div class="fp-drag" onclick="closeFP()"></div>
  <div class="flex items-center justify-between mb-4">
    <div class="font-display font-bold" style="color:var(--w);font-size:1.05rem">الفلاتر</div>
    <div class="flex items-center gap-2">
      <button class="text-[.72rem] font-bold" style="color:var(--t);background:none;border:none;cursor:pointer;font-family:inherit" onclick="resetFilters()">مسح الكل</button>
      <button onclick="closeFP()" style="width:28px;height:28px;border-radius:50%;
        background:var(--ta08);border:1px solid var(--ta12);cursor:pointer;
        display:flex;align-items:center;justify-content:center;color:var(--w60)">
  ...
  <div class="mb-5">
    <div class="text-[.65rem] font-bold tracking-[.14em] uppercase mb-3" style="color:var(--w40)">التصنيف</div>
    <div class="flex flex-wrap gap-2" id="fp-cat-chips">
      <button class="chip on" data-cat="all" onclick="fpCat(this,'all')">الكل</button>
      <button class="chip" data-cat="career" onclick="fpCat(this,'career')">المسار المهني</button>
      <button class="chip" data-cat="sales" onclick="fpCat(this,'sales')">مبيعات</button>
      <button class="chip" data-cat="hr" onclick="fpCat(this,'hr')">موارد بشرية</button>
      <button class="chip" data-cat="real" onclick="fpCat(this,'real')">عقارات</button>
      <button class="chip" data-cat="lang" onclick="fpCat(this,'lang')">لغات</button>
      <button class="chip" data-cat="edu" onclick="fpCat(this,'edu')">تعليم</button>
    </div>
  </div>
  ...
  <div class="mb-6">
    ...
    <div class="flex flex-col gap-2.5">
      <label class="flex items-center gap-3 cursor-pointer"><input type="radio" name="rt2" value="all" checked class="accent-[#00d4b8]" onchange="filterTime('all')"><span class="text-sm" style="color:var(--w60)">أي وقت</span></label>
      <label class="flex items-center gap-3 cursor-pointer"><input type="radio" name="rt2" value="short" class="accent-[#00d4b8]" onchange="filterTime('short')"><span class="text-sm" style="color:var(--w60)">سريع — أقل من ٨ د</span></label>
      <label class="flex items-center gap-3 cursor-pointer"><input type="radio" name="rt2" value="long" class="accent-[#00d4b8]" onchange="filterTime('long')"><span class="text-sm" style="color:var(--w60)">مطوّل — أكثر من ١٠ د</span></label>
    </div>
  </div>

  <button class="btn-p w-full py-3 text-sm font-bold" onclick="closeFP()">تطبيق الفلاتر</button>
</div>
...
function applyLang(){var h=document.documentElement;h.lang=isEn?'en':'ar';h.dir=isEn?'ltr':'rtl';document.querySelectorAll('[data-en],[data-ar]').forEach(function(el){var v=isEn?el.dataset.en:el.dataset.ar;if(!v)return;if(el.tagName==='INPUT'){el.placeholder=v;return;}if(el.children.length===0||Array.from(el.children).every(c=>c.tagName==='SVG'))el.textContent=v;});}
(()=>{if(localStorage.getItem('hc-lang')==='ar'){isEn=false;applyLang();}})();
window.addEventListener('scroll',()=>{var sp=document.getElementById('spbar');if(sp){var t=document.documentElement.scrollHeight-window.innerHeight;sp.style.width=(t>0?window.scrollY/t*100:0)+'%';}var n=document.getElementById('hc-nav');if(!n)return;var l=document.documentElement.classList.contains('light');n.style.background=window.scrollY>40?(l?'#fff':'rgba(3,13,23,.98)'):(l?'#fff':'rgba(3,13,23,.96)');n.style.borderColor=window.scrollY>40?(l?'#e5e7eb':'rgba(255,255,255,.12)'):(l?'#e5e7eb':'rgba(255,255,255,.08)');try{var scrollY=window.scrollY;}catch(e){}},{passive:true});
...
document.addEventListener('keydown',e=>{if(e.key==='Escape'){closeSearch();closeFP();}});

// ── Filter state ──────────────────────────────────────────────────────────
var activecat='all', activetime='all', activeq='';

function doSearch(q){activeq=q.toLowerCase();applyFilter();}

// Sidebar cat (checkbox style)
function filterCat(cat, labelEl){
  activecat=cat;
  // Reset all indicators
  document.querySelectorAll('.cat-indicator').forEach(function(el){
    var c=el.id.replace('ci-','');
    var catData={'all':{'bc':'var(--t)','bg':'transparent','icon':false},'career':{'bc':'#2de0ca55','bg':'transparent','icon':false},'sales':{'bc':'#60a5fa55','bg':'transparent','icon':false},'hr':{'bc':'#c084fc55','bg':'transparent','icon':false},'real':{'bc':'#fb923c55','bg':'transparent','icon':false},'lang':{'bc':'#34d39955','bg':'transparent','icon':false},'edu':{'bc':'#f472b655','bg':'transparent','icon':false}};
    el.innerHTML='';
    var d=catData[c];
    if(d){el.style.borderColor=d.bc;el.style.background=d.bg;}
  });
  // Set active
  var active=document.getElementById('ci-'+cat);
  if(active){
    var acols={'all':'var(--t)','career':'#2de0ca','sales':'#60a5fa','hr':'#c084fc','real':'#fb923c','lang':'#34d399','edu':'#f472b6'};
    var col=acols[cat]||'var(--t)';
    active.style.borderColor=col;active.style.background=col;
    active.innerHTML='<svg class="w-2.5 h-2.5" viewBox="0 0 10 10" fill="none" stroke="#020e18" stroke-width="2.5"><path d="M1.5 5l2.5 2.5 4.5-4.5"/></svg>';
  }
  // Sync chip bar
  document.querySelectorAll('#chip-bar .chip').forEach(c=>c.classList.remove('on'));
  var chip=document.querySelector('#chip-bar .chip[data-cat="'+cat+'"]');
  if(chip)chip.classList.add('on');
  // Sync popup chips
  document.querySelectorAll('#fp-cat-chips .chip').forEach(c=>c.classList.remove('on'));
  var fpChip=document.querySelector('#fp-cat-chips .chip[data-cat="'+cat+'"]');
  if(fpChip)fpChip.classList.add('on');
  applyFilter();
}

// Chip bar cat
function chipCat(el,cat){
  document.querySelectorAll('#chip-bar .chip').forEach(c=>c.classList.remove('on'));
  el.classList.add('on');
  filterCat(cat,null);
}
...
function filterTime(t){
  ...
  document.querySelectorAll('input[name="rt"],input[name="rt2"]').forEach(function(r){
    r.checked=r.value===t;
  });
  applyFilter();
}
...
function resetFilters(){
  activecat='all';activetime='all';activeq='';
  var inp=document.getElementById('search-inp');if(inp)inp.value='';
  filterCat('all',null);
  document.querySelectorAll('input[name="rt"],input[name="rt2"]').forEach(function(r,i){r.checked=i%3===0;});
  applyFilter();
}
...
function applyFilter(){
  ...
  cards.forEach(function(c){
    ...
    var qOk=!activeq||c.innerText.toLowerCase().includes(activeq);
    var timeOk=true;
    if(activetime!=='all'){
      var rt=(c.querySelector('[style*="⏱"]')||c).innerText;
      var num=parseInt(rt.replace(/[^٠-٩0-9]/g,'').replace(/[٠-٩]/g,d=>d.charCodeAt(0)-1632));
      if(activetime==='short')timeOk=num<8;
      if(activetime==='long')timeOk=num>=10;
    }
    var show=catOk&&qOk&&timeOk;
    c.style.display=show?'':'none';
    if(show)n++;
  });
  var cnt=document.getElementById('art-count');if(cnt)cnt.textContent=n;
  var emp=document.getElementById('empty');if(emp)emp.style.display=n?'none':'block';
  // Update sidebar counts
  var catMap={};
  document.querySelectorAll('.art-card:not([style*="display: none"]):not([style*="display:none"])').forEach(function(c){
    catMap[c.dataset.cat]=(catMap[c.dataset.cat]||0)+1;
  });
  var total=0;Object.values(catMap).forEach(v=>total+=v);
  var allEl=document.getElementById('cnt-all');if(allEl)allEl.textContent=total;
  ['career','sales','hr','real','lang','edu'].forEach(function(cat){
    var el=document.getElementById('cnt-'+cat);
    if(el)el.textContent=catMap[cat]||'';
  });
  // Update chip all count
  var chipAll=document.querySelector('#chip-bar .chip[data-cat="all"]');
  if(chipAll)chipAll.innerHTML='الكل ('+(n)+')';
  // Update filter badge
  var nb=(activecat!=='all'?1:0)+(activetime!=='all'?1:0)+(activeq?1:0);
...
(function(){
  ...
  panel.addEventListener('touchmove',function(e){if(e.touches[0].clientY-startY>70)closeFP();},{passive:true});
})();
